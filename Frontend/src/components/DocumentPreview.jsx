...
const DocumentPreview = ({ documentUuid, onRemove, showExpiry = false }) => {
  ...
  useEffect(() => {
    const fetchDocumentInfo = async () => {
      try {
        console.log("DocumentPreview: Fetching info for UUID:", documentUuid);
        ...
        if (response.ok) {
          ...
          console.log("DocumentPreview: Received data:", data);
          setDocumentInfo(data);
        } else {
          console.log("DocumentPreview: Failed to fetch:", response.status, response.statusText);
        ...
        <div className="flex-1 min-w-0 w-full">
          ...
          <div className="mt-1 space-y-1">
            ...
            <p className="text-xs text-gray-500">
              {formatFileSize(documentInfo.doc_size)} • {documentInfo.file_extension ? documentInfo.file_extension.toUpperCase() : 'N/A'}
            </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {documentInfo.download_url ? (
              ...
              <div className="text-sm text-gray-500 italic">
                No downloadable file (virtual document)
              </div>
            )}
