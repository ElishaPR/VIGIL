      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600">{reminder.expiry_date ? formatDate(reminder.expiry_date) : "No expiry"}</span>
    ...
    <div className="p-4 rounded-xl border border-gray-200 bg-white transition-all hover:shadow-md">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {doc.expiry_date && (
            <p className="text-sm text-gray-600 mt-2">Expires {formatDate(doc.expiry_date)}</p>
          )}
        </div>
      </div>
    </div>
    ...
    const fetchAll = async () => {
      try {
        ...
        const [userRes, remindersRes, docsRes] = await Promise.all([
          ...
          fetch("http://localhost:8000/documents/list", { credentials: "include" }),
        ]);
