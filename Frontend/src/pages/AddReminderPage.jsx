...
export function AddReminderPage() {
  ...
  const validateForm = () => {
    ...
    if (scheduleType === "custom") {
      if (!reminderAt) {
        newErrors.reminderAt = "Reminder date is required for custom schedule.";
      } else {
        const reminder = new Date(reminderAt);
        const expiry = new Date(expiryDate);
        if (reminder > expiry) {
          newErrors.reminderAt = "Reminder date cannot be after expiry date.";
        }
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (scheduleType === "custom" && reminderAt) {
        formData.append("reminder_at", new Date(reminderAt).toISOString());
      }
